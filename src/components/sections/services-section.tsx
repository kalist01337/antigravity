        ...
        <div className="mx-auto grid max-w-[1200px] grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {siteConfig.services.map((service, index) => {
            ...
            return (
              <article
                key={service}
                className="group relative flex min-h-[160px] flex-col justify-between overflow-hidden rounded-2xl card-premium p-6"
              >
                {/* Subtle top glow */}
                ...
                <div className="relative z-10">
                  <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-surfaceSoft text-brand shadow-[inset_0_0_10px_rgba(255,255,255,0.05)] ring-1 ring-inset ring-brand/20 transition-all duration-500 group-hover:bg-brand/10 group-hover:scale-110 group-hover:ring-brand/40 group-hover:shadow-[0_0_15px_rgba(197,160,89,0.2)]">
                    <div className="scale-75">{Icon}</div>
                  </div>

                  <h3 className="font-serif font-bold text-[1rem] leading-snug text-ink transition-colors group-hover:text-brandSoft md:text-[1.1rem]">
                    {service}
                  </h3>
                </div>
